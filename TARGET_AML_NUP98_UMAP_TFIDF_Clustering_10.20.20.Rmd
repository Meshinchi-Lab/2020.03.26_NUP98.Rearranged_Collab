...
```{r}
updated.IDs <- read.csv(file.path(PROJHOME,"0000.00.02_Reference_GeneInfo/GeneSymbol_Ensembl_ID_Conversion_GRCh37_v69_to_v102.csv")) %>% 
...
```{r eval=FALSE}
TPMs <- readRDS(file.path(PROJHOME, "0000.00.03_ExpressionMatrices/BCCA_GRCh37_Ensembl_v69/TARGET_AML_MPN_DS_NBM_3044Samples_Ribodepleted_RNAseq_geneLevel_dupGenesRemoved_TPM.RDS"))
...
umap_NUP98 <- readRDS("UMAP/NUP98_Only/TARGET_AML_sg8780_NUP98.Rearr_log2Counts_UMAP_NUP98_Results_7.16.20.RDS")
...
# cc.clusters
cc.clusters <- list()
cc.clusters$cluster_k5 <- brewer.pal(n=9,"Set1")[-2][1:5]
cc.clusters
```
...
p.NUP98 <- plot_ly() %>% 
  ...
  add_trace(data=res,
            x = ~x, y = ~y, z = ~z,
            color =  ~NUP98.Rearranged.Groups, #~cluster_k5,
            colors = cc$NUP98.Rearranged.Groups, #cc.clusters$cluster_k5,
            type='scatter3d',
            mode='markers',
            text=info,
            hoverinfo='text',
...
# NUP98 Figures for Manuscrpt 
      
```{r fig.height=3}
flevels <- rev(c("NUP98-X", "NUP98-NSD1","NUP98-KDM5A",
                  "CBFA2T3-GLIS2","del5q","monosomy7",
                    "RUNX1-RUNX1T1",
                    "CBFB-MYH11", 
                    "AML",
                    "KMT2A",
...
res <- umap_allDX$umap_res %>%
  ...
  #               . %in% c("No.Primary.Fusion.CNV","AML") ~ "FLT3-ITD",
  #             NPM.mutation. == "Yes" &
  #               . %in% c("No.Primary.Fusion.CNV","AML") ~ "NPM1",
  #             TRUE ~ .)) %>%
  mutate_at(vars(NUP98.Rearranged),
...
NUP98_colors["NBM"] <- 'black'
# NUP98_colors <- NULL
# NUP98_colors["No.Primary.Fusion.CNV"] <- "azure4"
# NUP98_colors["AML"] <- "grey80"
# NUP98_colors["NPM1"] <- "#FDDAEC"
# NUP98_colors["FLT3-ITD"] <- "#B3CDE3" #"#377EB8"
# NUP98_colors["KMT2A"] <- "khaki2"
# NUP98_colors["DEK-NUP214"] <-  "aquamarine3" #"darkslategray3"
# NUP98_colors["CBFB-MYH11"] <- "burlywood"
# NUP98_colors["CBFA2T3-GLIS2"] <- "firebrick3"
# NUP98_colors["RUNX1-RUNX1T1"] <- "sienna3"
# NUP98_colors["NUP98-KDM5A"] <- "navy"
# NUP98_colors["NUP98-NSD1"] <- "cornflowerblue"
# NUP98_colors["Single Mutant"] <- "magenta"
# NUP98_colors["Double Mutant"] <- "green2"
...
ellipse_dat <- res %>% 
  filter(grepl("RUNX1|DEK|NUP98|Mutant|KMT2A|CBFB|GLIS2|NPM1|FLT3",
               NUP98.Double.Single.Allelic.Status)) %>% 
  mutate(ellipse_groups=ifelse(grepl("Mutant",as.character(NUP98.Double.Single.Allelic.Status)),"NUP98", as.character(NUP98.Double.Single.Allelic.Status))) %>% 
  droplevels()
...
NUP98_dim12 <- ggplot(res,
                      aes(x=x,y=y,color=NUP98.Double.Single.Allelic.Status)) +
...
NUP98_dim12
# dev.off()
# saveRDS(NUP98_dim12,"TARGET_AML_sg6816_UMAP_dim1_2Dscatter.RDS")
```

```{r fig.height=7, fig.width=20}
NUP98_dim13 <- ggplot(res,shape=21,
                      aes(x=x,y=z,color=NUP98.Double.Single.Allelic.Status)) +
   stat_ellipse(data=ellipse_dat,
                   aes(x=x,y=z, fill=NUP98.Double.Single.Allelic.Status),
                   geom = "polygon",
                   size=1.5,
                   alpha=0.25,
                   level = 0.75,
...
# pdf("TARGET_AML_sg6816_NUP98_UMAP_dim123.pdf", height = 7, width = 20)
grid.arrange(grobs=list(NUP98_dim12,NUP98_dim13),
             ncol=2, widths=c(1,1.45))
# dev.off()

# saveRDS(arrangeGrob(grobs=list(NUP98_dim12,NUP98_dim13),
#              ncol=2, widths=c(1,1.45)), "TARGET_AML_sg6816_NUP98_UMAP_dim123_2Dscatter.RDS")
...
sm_only <- mutate(ellipse_dat,
                  sm=ifelse(NUP98.Double.Single.Allelic.Status=="Single Mutant",
                            "Single Mutant", NA))
other_aml <- mutate(res,
                    aml=ifelse(NUP98.Double.Single.Allelic.Status %in%
...
p <- ggplot(data=ellipse_dat, 
            mapping=aes(x=x, y=y,z=z, 
                        # color=NUP98.Double.Single.Allelic.Status,
                        fill=NUP98.Double.Single.Allelic.Status)) +
...
```{r}
output="png"
# ggsave(filename = paste0("TARGET_AML_sg6816_NUP98_UMAP_FLT3_NPM1_withGreyPoints_brightNUP98_3Dscatter.",
#                          output),
#        plot=rotated,
#        height = 7, width=10,
#        units="in",
...
# saveRDS(rotated,"NUP98_UMAP_3Dscatter.RDS")
```

# Expression of gene partners 

```{r}
gene_partners <- read.csv("TARGET_AML_NUP98_gene_pairs.csv")
