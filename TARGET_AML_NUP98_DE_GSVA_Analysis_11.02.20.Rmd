...
```{r}
forTim <- CDEs %>% 
  select(USI,matches("_chr13|_13"))

# head(forTim)
# table(forTim$Any_chr13_Abnormality, forTim$monosomy_trisomy_13)
# write.csv(forTim,"TARGET_AML_chr13_abnormalities_by_Karyotype_allPatients_08.30.21.csv", row.names = FALSE)
```

```{r}
# CDEs %>% 
#   filter(Any_chr13_Abnormality=="chr13_Abnormality", !grepl("NUP98", NUP98.Rearranged.Groups)) %>% 
...
# CDEs %>% 
#   filter(Any_chr13_Abnormality=="chr13_Abnormality", !grepl("NUP98", NUP98.Rearranged.Groups)) %>% 
#   mutate(KMT2A=ifelse(grepl("KMT2A", Primary.Fusion), "KMT2A", "")) %>% 
#   group_by(KMT2A) %>% 
#   count() %>% 
#   arrange(desc(n))
# 
...
There are 12/47 KMT2A (25.5%), 8/47 (17.0%) RUNX1-RUNX1T1 with chr13 alterations and Non-NUP98 translocated

```{r}
# #file.path(CDE, "Merged/00_Old/TARGET_AML_0531_1031_merged_CDEs_03.17.21.csv")
# CDEs <- read.csv(file.path(CDE, "Merged/TARGET_AML_0531_1031_merged_CDEs_05.21.21.csv"),
#                    na.strings = c("N/A","#N/A","NA","^$", "^\\.$")) %>% 
#   mutate(Reg.=as.character(Reg.)) %>% 
#   inner_join(., mutate(NUP98.cohort, Reg.=as.character(Reg.)) %>% 
#                select(-USI),
#              by="Reg.") %>% 
#   filter(Eligibility_Comments != "remove")
# 
...
The interaction of HOXA9, HOXB8, and HOXA10 with Pbx1 and Meis1 is essential for immortalization and differentiation of myeloid progenitors (41–44). These DNA binding complexes may either activate or repress target genes depending on temporal or spatial expression (41–43, 45).
...
```{r}
rownames(core.genes.subset)
any(cyt.genes %in% kegg.all.sym$`hsa05340 Primary immunodeficiency`)
```
...
# pdf("Figures/TARGET_AML_Cytolytic_Score_violinplot.pdf", height = 5, width = 10)
ggplot(cyt.scores, aes(x=NUP98.Rearranged.Groups.Addl, y=cytolytic.score, 
                       color=NUP98.Rearranged.Groups.Addl)) +
  ggbeeswarm::geom_quasirandom(aes(color=NUP98.Rearranged.Groups.Addl ), #monocytic_monoblastic did not have a strong correlation to the increased cyt score 
                               alpha=1.0) +
  geom_violin(aes(fill=NUP98.Rearranged.Groups.Addl), color="black", draw_quantiles = c(0.5),
              alpha=0.2) +
  # geom_boxplot(aes(fill=NUP98.Rearranged.Groups.Addl),
  #             color="black", alpha=0.2, outlier.shape = NA) +
  scale_color_manual(values=cc_nup_addl) +
  scale_fill_manual(values=cc_nup_addl) +
